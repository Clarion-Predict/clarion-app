-- Record when a market was resolved.
--
-- The admin console used to ask for an optional cutoff time whose real job was
-- voiding bets placed after it -- not recording a resolution date. The console
-- no longer asks, so p_cutoff now arrives null and nothing is voided; the
-- voiding branch is left in place for when a market genuinely needs it.
--
-- Body below is the deployed function verbatim, with resolved_at added to the
-- markets update.
