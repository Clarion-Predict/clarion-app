...
// ========== SEARCH MODAL ==========
const SearchModal = ({ onClose, communityUsers, onFollowToggle, onViewProfile, authUser }) => {
      ...
      <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl" onClick={e => e.stopPropagation()}>
        <div className="p-4 border-b border-stone-100">
          <div className="flex items-center gap-3">
            ...
            />
            <button onClick={onClose} className="text-stone-400"><X className="w-4 h-4" /></button>
          </div>
        </div>
        <div className="max-h-80 overflow-y-auto">
          {query.trim().length === 0 && (
            <div className="p-6 text-center text-sm text-stone-400">Type a username to search</div>
          )}
          {query.trim().length > 0 && results.length === 0 && (
            <div className="p-6 text-center text-sm text-stone-400">No users found for "{query}"</div>
          )}
          {results.map(u => (
            <div key={u.id} className="flex items-center gap-3 px-4 py-3 hover:bg-stone-50 border-b border-stone-50 last:border-0">
              <div
              ...
              <div className="flex-1 min-w-0 cursor-pointer" onClick={() => { onViewProfile(u); onClose(); }}>
                <div className="text-sm font-medium text-stone-900">@{u.username}</div>
  ...
  supabase.auth.getSession().then(async ({ data: { session } }) => {
    if (session?.user) {
      ...
      if (balanceRow) {
        ...
        const shouldRefill = isMonday && lastRefill !== todayStr && balanceRow.balance < 100;
        if (shouldRefill) {
          const newBalance = 100;
          await supabase.from('balances').update({ balance: newBalance }).eq('user_id', session.user.id);
...
if (authLoading) {
  ...
  setMarkets(prev => prev.map(m => m.id === selectedMarket.id ? {
    ...m,
    yes: newYesPrice,
    no: newNoPrice,
  ...
  return (
    <div className="min-h-screen bg-amber-50/40 pb-24 md:pb-6">
      ...
      {showSearch && <SearchModal onClose={() => setShowSearch(false)} communityUsers={communityUsers} onFollowToggle={handleFollowToggle} onViewProfile={setViewingProfile} authUser={authUser} />}
